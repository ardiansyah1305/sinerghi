...
class PenugasanController extends BaseController
{
    ...
    private const TANGGAL_MULAI_PENGAJUAN = 15;    // Tanggal mulai pengajuan setiap bulan
    private const TANGGAL_AKHIR_PENGAJUAN = 20;    // Tanggal berakhir pengajuan setiap bulan
    private const BATAS_PERPANJANGAN = 2;          // Jumlah hari perpanjangan jika jatuh di weekend
    // =========================================
    ...
    public function storeUsulan()
    {
        try {
            ...
            if (!$this->isWithinSubmissionPeriod()) {
                return $this->response->setJSON([
                    'status' => 'error',
                    'message' => 'Pengajuan hanya dapat dilakukan dari tanggal 15 hingga 20. Jika tanggal 20 jatuh pada hari Sabtu atau Minggu, pengajuan dapat dilakukan hingga hari Senin berikutnya.'
                ]);
            }
            ...
            foreach ($rows as $key => $row) {
                ...
                // Cek jenis alokasi untuk tanggal yang sama
                if (!isset($alokasiPerTanggal[$tanggalKerja])) {
                    $alokasiPerTanggal[$tanggalKerja] = $jenisAlokasi;
                } else if ($alokasiPerTanggal[$tanggalKerja] !== $jenisAlokasi) {
                    $errors[] = "Baris " . ($key + 1) . ": Tanggal $tanggalKerja sudah memiliki jenis alokasi " .
                        $alokasiPerTanggal[$tanggalKerja] . ". Tidak boleh ada dua jenis alokasi berbeda pada tanggal yang sama.";
                    continue;
                }
    ...
    public function ajukanUsulan()
    {
        ...
        if (!$usulan_id || !$bulanPengajuan || !$tahunPengajuan) {
    ...
    public function validateExcel()
    {
        try {
            ...
            foreach ($rows as $key => $row) {
                ...
                $tanggalKerja = isset($row[2]) ? trim((string)$row[2]) : '';
                $jenisAlokasi = isset($row[3]) ? strtoupper(trim((string)$row[3])) : '';
                ...
                if (!isset($alokasiPerTanggal[$tanggalKerja])) {
                    ...
                    $errors[] = "Baris " . ($key + 2) . ": Tanggal $tanggalKerja sudah memiliki jenis alokasi " .
                        $alokasiPerTanggal[$tanggalKerja] . ". Tidak boleh ada dua jenis alokasi berbeda pada tanggal yang sama.";
                    continue;
                }
            ...
            if (!empty($missingPegawai)) {
                ...
                foreach ($missingPegawai as $p) {
                    $errorMsg .= "- {$p['nama']} (NIP: {$p['nip']})<br>";
                }
                $errors[] = $errorMsg;
            }
    ...
    {
        $tanggalSekarang = date('d');
        $hariSekarang = date('N'); // 1 (Senin) sampai 7 (Minggu)
        
        // Jika tanggal akhir pengajuan jatuh pada hari Sabtu atau Minggu
        if ($tanggalSekarang == self::TANGGAL_AKHIR_PENGAJUAN) {
            if ($hariSekarang == 6 || $hariSekarang == 7) { // Sabtu atau Minggu
                return true; // Bisa mengajukan sampai hari Senin
            }
        }
        
        // Jika dalam masa perpanjangan dan hari Senin
        if (($tanggalSekarang > self::TANGGAL_AKHIR_PENGAJUAN) && 
            ($tanggalSekarang <= self::TANGGAL_AKHIR_PENGAJUAN + self::BATAS_PERPANJANGAN) && 
            $hariSekarang == 1) {
            return true;
        }
        
        // Periode normal
        return $tanggalSekarang >= self::TANGGAL_MULAI_PENGAJUAN && 
               $tanggalSekarang <= self::TANGGAL_AKHIR_PENGAJUAN;
    }
}
