...
foreach ($unit_kerja as $uk) {
    $uk['type'] = 'kerja'; // Menandai data dari unit_kerja
    $combined_data[] = $uk;
}
foreach ($unit_kerjaa as $uka) {
    $uka['type'] = 'kerjaa'; // Menandai data dari unit_kerjaa
    $combined_data[] = $uka;
}

usort($combined_data, function($a, $b) {
    return $a['unit_kerja_id'] <=> $b['unit_kerja_id']; // Urutkan sesuai kebutuhan
});
...
foreach ($combined_data as $item): ?>
    <tr>
        ...
        <td class="text-center border"><?= $item['type'] == 'kerja' ? $item['nama_unit_kerja'] : $item['nama_unit_kerjaa']; ?></td>
        <td class="text-center border"><?= $item['parent_id']; ?></td>
        <td class="text-center border"><?= $item['type'] == 'kerja' ? $item['unit_kerja_id'] : $item['unit_kerjaa_id']; ?></td>
        <td class="text-center border">
            <div class="d-flex justify-content-center gap-3 custom-action-buttons">
                <button class="btn btn-warning btn-sm custom-edit-btn border" data-bs-toggle="modal" data-bs-target="#editPegawaiModal<?= $item['id']; ?>">Edit</button>
                <button onclick="confirmDelete(<?= $item['id']; ?>)" class="btn btn-danger btn-sm custom-delete-btn border">Hapus</button>
            </div>
        </td>
    </tr>
