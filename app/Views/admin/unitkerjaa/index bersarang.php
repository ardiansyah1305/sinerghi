...
<div class="container-fluid px-5 py-4">
    ...
    <tr>
        ...
        <td class="text-center border"><?= $uk['nama_unit_kerja']; ?></td>
    ...
    <!-- Mengambil data dari $unit_kerjaa berdasarkan ID -->
    <?php foreach ($unit_kerjaa as $uka): ?>
        <?php if ($uka['unit_kerja_id'] == $uk['id']): ?>
            <tr>
                <td class="text-center border"><?= $i++; ?></td>
                <td class="text-center border"><?= $uka['nama_unit_kerja']; ?></td>
                <td class="text-center border"><?= $uka['unit_kerja_id']; ?></td>
                <td class="text-center border">
                    <div class="d-flex justify-content-center gap-3 custom-action-buttons">
                        <button class="btn btn-warning btn-sm custom-edit-btn border" data-bs-toggle="modal" data-bs-target="#editPegawaiModal<?= $uka['id']; ?>">Edit</button>
                        <button onclick="confirmDelete(<?= $uka['id']; ?>)" class="btn btn-danger btn-sm custom-delete-btn border">Hapus</button>
                    </div>
                </td>
            </tr>
        <?php endif; ?>
